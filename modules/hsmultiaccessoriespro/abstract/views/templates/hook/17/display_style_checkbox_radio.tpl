...
<table id="product_list_accessory_{$group.id_accessory_group|intval}" class="accessorygroup clear">
   ...
   <input data-id-product-attribute ="{if $accessory.id_product_attribute != 0}{$accessory.id_product_attribute|intVal}{else}{$accessory.default_id_product_attribute|intVal}{/if}" data-randomId ="{$accessory.random_product_accessories_id|escape:'htmlall':'UTF-8'}" {if $is_checked == 1} checked="checked" data-quang="1" {if $is_disabled == 1} disabled='disabled' {/if}{/if} data-required-buy-together ="{$accessory.is_available_buy_together|intval}" type="{$input_type|escape:'htmlall':'UTF-8'}" id='accessories_proudct_{$group.id_accessory_group|escape:'htmlall':'UTF-8'}_{$accessory.id_accessory|escape:'htmlall':'UTF-8'}' class="accessory_item custom-control-input" value="{$accessory.id_accessory|escape:'htmlall':'UTF-8'}" {if $accessory.is_available_for_order} disabled="disabled"{/if} {if $group.display_style == HsMaDisplayStyle::RADIO}name="accessories_{$group.id_accessory_group|intval}"{/if}/>
  <label class="custom-control-label" for="accessories_proudct_{$group.id_accessory_group|escape:'htmlall':'UTF-8'}_{$accessory.id_accessory|escape:'htmlall':'UTF-8'}" ></label>
</div>
            ...
            <td>
                <form action="{$urls.pages.cart}" class="add_accessory_to_cart">
                    ...
                    {$accessory.description_short nofilter}
                    </span>
                    ...
                    {if $accessory_configuration_keys.HSMA_SHOW_CUSTOM_QUANTITY}
                        <input class="custom_quantity" name='qty' style="display:none;" {if !$accessory_configuration_keys.HSMA_ALLOW_CUSTOMER_CHANGE_QTY} disabled="disabled" {/if} data-custom-quantity="{$accessory.default_quantity|intval}" type="number" name="quantity" id="quantity_{$group.id_accessory_group|escape:'htmlall':'UTF-8'}_{$accessory.id_accessory|escape:'htmlall':'UTF-8'}" value="{$accessory.default_quantity|intval}" min='{$accessory.min_quantity|intval}'/>
                    {/if}

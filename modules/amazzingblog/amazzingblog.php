...
class AmazzingBlog extends Module
{
    ...
    public function defineCustomSlug()
    {
        ...
        foreach ($data as $d) {
            ...
            $this->slug_multilang[$d['id_lang']] = $d['link_rewrite'];
        }
    }

    public function getSettings($type, $id_shop = false, $force_query = false)
    {
        $id_shop = $id_shop ? $id_shop : $this->id_shop;
        if (empty($this->settings[$type]) || $force_query) {
            $settings = $this->db->getValue('
                SELECT value FROM '._DB_PREFIX_.'a_blog_settings
                WHERE name = \''.pSQL($type).'\' AND id_shop = '.(int)$id_shop.'
            ');
            ...
            if ($type == 'comment' && !function_exists('curl_init')) {
                $this->settings[$type]['captcha'] = 0;
            }
        }
        return $this->settings[$type];
    }

    public function validateField($field, $other_submitted_data)
    {
        $validated = true;
        if (!empty($field['required_if']) && !empty($other_submitted_data[$field['required_if']])) {
            $field['required'] = 1;
        }
        if ($field['value'] == '') {
            $validated = empty($field['required']);
        } elseif (!empty($field['validate'])) {
            $validated = false;
            $rule = $field['validate'];
            if (is_callable(array('Validate', $rule))) {
                $validated = Validate::$rule($field['value']);
            } elseif ($rule == 'isAvatarSize') {
                $size = explode('*', $field['value']);
                $validated = $size[0] && isset($size[1]) && $size[1] &&
                Validate::isInt($size[0]) && Validate::isInt($size[1]);
            }
        }
        if (!$validated) {
            $this->throwError($field['display_name'].': '.$this->l('Incorrect value'));
        }
    }

    public function saveSettings($type, $submitted_data = array(), $forced_shop_ids = array())
    {
        $to_save = array();
        if ($required_settings = $this->getSettingsFields($type)) {
            foreach ($required_settings as $name => $field) {
                $field['value'] = isset($submitted_data[$name]) ? $submitted_data[$name] : $field['value'];
                $this->validateField($field, $submitted_data);
                $to_save[$name] = $field['value'];
            }
            $rows = array();
    ...
    public function getTables()
    {
        $tables = array(
            ...
            'a_blog_img_type' => '(
                id_img_type int(10) unsigned NOT NULL AUTO_INCREMENT,
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        if (Tools::getValue('configure') == $this->name) {
            ...
            // include newer version of mce, without bugs
            $this->context->controller->js_files[] = '//cdnjs.cloudflare.com/ajax/libs/tinymce/4.7.6/tinymce.min.js';
        } elseif (!$this->is_17 && Tools::getValue('controller') == 'AdminProducts') {
            $this->context->controller->addJquery();
            $this->context->controller->addJqueryUI('ui.sortable');
        }
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('ajax') && Tools::isSubmit('action')) {
            ...
            $this->verifyMethod($action_method);
            $this->$action_method();
        }
        ...
        $mce_additional_css_files = array(
            ...
            $this->_path.'views/css/front.css?'.$this->version,
            _THEME_CSS_DIR_.($this->is_17 ? 'theme' : 'global').'.css',
        );
        $js_strings = array(
    ...
    public function importData($zip_file = false)
    {
        ...
        foreach ($tables as $table_name) {
            ...
            } elseif ($table_name == 'a_blog_category') {
                foreach ($imported_data[$table_name] as $row) {
                    foreach (array('cover', 'main_img') as $name) {
                        $row[$name] = isset($row[$name]) ? $row[$name] : ''; // retro compatibility
                    }
                    $rows[] = $row;
                }
            } else {
        ...
        if ($imported = $this->runSql($sql)) {
            ...
            $settings = array('general', 'post', 'postlist', 'category', 'comment');
            foreach ($shop_ids as $id_shop) {
            ...
            if (!$this->getImgTypes() && $retro_img_settings = $this->getSettings('img')) {
                $this->convertImgSettingsToNewFormat($retro_img_settings);
            }

            // update settings data for the new form
            $this->general_settings = $this->getSettings('general', $this->id_shop, true);
    ...
    public function ajaxSaveImgType()
    {
        $values = Tools::getValue('values');
        exit(Tools::jsonEncode($this->saveImgType($values, true)));
    }

    public function saveImgType($values, $ajax = false)
    {
        if ($original_dir = $this->db->getValue('
                SELECT dir FROM '._DB_PREFIX_.'a_blog_img_type
                WHERE id_img_type = '.(int)$values['id_img_type'].'
            ')) {
            $values['dir'] = $original_dir; // make sure dir is not changed
        }
        // validate fields
        if ($this->db->getValue('
                SELECT id_img_type FROM '._DB_PREFIX_.'a_blog_img_type
                WHERE dir = \''.pSQL($values['dir']).'\'
                AND id_img_type != '.(int)$values['id_img_type'])) {
            $this->throwError(sprintf($this->l('"%s" directory is already used by another type'), $values['dir']));
        }
        $values_to_save = $field_errors = array();
        foreach ($this->getTableColumns('a_blog_img_type') as $name) {
            $value = isset($values[$name]) ? $values[$name] : '';
            if ($value == '' || ($name == 'name' && pSQL($value) != $value) ||
                (($name == 'width' || $name == 'height') && ((int)$value.'' != $value.'' || (int)$value <= 0))) {
                $field_errors[$name] = $name;
            }
            $values_to_save[$name] = $value;
        }
        $saved = $field_errors ? false : $this->db->execute('
            REPLACE INTO '._DB_PREFIX_.'a_blog_img_type
            VALUES (\''.implode('\', \'', array_map('pSQL', $values_to_save)).'\')
        ');
        $id = $values['id'] ? $values['id'] : $this->db->Insert_ID();
        if ($ajax) {
            $ret = array('saved' => $saved, 'id' => $id);
            if ($field_errors) {
                $ret['field_errors'] = $field_errors;
            }
        } else {
            $ret = $saved;
        }
    ...
    {
        $columns = array();
        foreach ($this->db->executeS('SHOW COLUMNS FROM '._DB_PREFIX_.pSQL($table_name)) as $c) {
            $columns[] = $c['Field'];
        }
        return $columns;
    }

    public function ajaxCallImgTypeRow()
    {
        $img_types = array();
        foreach ($this->getTableColumns('a_blog_img_type') as $name) {
            $img_types[0][$name] = '';
        }
        $this->context->smarty->assign(array('img_types' => $img_types));
        $html = $this->display(__FILE__, 'views/templates/admin/img-types.tpl');
        $row_html = '';
        if ($row_begin = (int)Tools::strpos($html, '<!-- rows -->')) {
            $row_begin += 13;
            $row_end = (int)Tools::strpos($html, '<!-- /rows -->') - $row_begin;
            $row_html = Tools::substr($html, $row_begin, $row_end).'';
        }
        $ret = array('row_html' => utf8_encode($row_html));
        exit(Tools::jsonEncode($ret));
    }

    public function validateScope($scope)
    {
        return ($scope == 'post' || $scope == 'category') ? $scope : false;
    }

    public function getImgTypes($scope = false)
    {
        $scope = $this->validateScope($scope);
        $types = $this->db->executeS('
            SELECT * FROM '._DB_PREFIX_.'a_blog_img_type
            '.($scope ? ' WHERE '.pSQL($scope).' = 1' : '').'
        ');
        return $types;
    }

    public function getClosestSize($w, $h, $scope)
    {
        $size_identifier = 'closest_size_'.$w.'_'.$h.'_'.$scope;
        if (!isset($this->$size_identifier)) {
            $scope = $this->validateScope($scope);
            $this->$size_identifier = $this->db->getValue('
                SELECT dir FROM '._DB_PREFIX_.'a_blog_img_type
                WHERE width >= '.(int)$w.' AND height >= '.(int)$h.'
                '.($scope ? ' AND '.pSQL($scope).' = 1' : '').'
                ORDER BY width ASC, height ASC
            ');
        }
        return $this->$size_identifier;
    }

    public function ajaxDeleteImgtype()
    {
        $id = Tools::getValue('id');
        $ret = array('deleted' => $this->deleteImgType($id));
        exit(Tools::jsonEncode($ret));
    }

    public function deleteImgType($id_img_type)
    {
        // delete existing images
        if ($type = $this->getImgTypeData($id_img_type)) {
            foreach (array('post', 'category') as $scope) {
                foreach (glob($this->img_dir_local.$this->getImgSubDir($scope).'/*', GLOB_ONLYDIR) as $dir) {
                    $this->recursiveRemove($dir.'/'.$type['dir'].'/');
                }
            }
        }
        return $this->db->execute('
            DELETE FROM '._DB_PREFIX_.'a_blog_img_type
            WHERE id_img_type = '.(int)$id_img_type.'
        ');
    }
    ...
    public function getDefaultListSettings($key = false)
    {
        $settings = array('npp' => 10, 'npp_options' => '5,10,20,50,100');
        if ($key) {
            $settings = isset($settings[$key]) ? $settings[$key] : false;
        }
        return $settings;
    }
    ...
    public function getPaginationSettings(
        ...
        $list_settings = $list_settings ? $list_settings : $this->getDefaultListSettings();
        if ($npp = Tools::getValue('npp')) {
            ...
            $npp = $list_settings['npp'];
        }
        $settings = array(
            'p' => $page ? $page : Tools::getValue('p', 1),
            'total' => Tools::getValue('total', $this->getTotal($resource, $additional_filters)),
            'npp' => $npp,
            'npp_options' => explode(',', $list_settings['npp_options']),
        );
        return $settings;
    }
    ...
    public function ajaxDisplayListItems()
    {
        ...
        $pagination_settings = $this->getPaginationSettings($resource, array(), $additional_filters);
    ...
    public function getImgSrc($resource_type, $resource_id, $size, $img_name)
    {
        $src = '';
        $path = $this->getImgSubDir($resource_type).'/'.$resource_id.'/'.$size.'/'.$img_name;
    ...
    {
        if ($settings['type'] == 'category_selected') {
            $imploded_ids = $this->formatIDs($settings['cat_ids']);
        } elseif ($settings['type'] == 'category_children') {
        ...
        $categories = $this->db->executeS('
            ...
            (!empty($settings['num']) ? ' LIMIT '.(int)$settings['num'] : '').'
        ');
        foreach ($categories as &$c) {
            $c['id'] = $c['id_category'];
            $c['url'] = $this->getCategoryLink($c['id_category'], $c['link_rewrite']);
            if (!empty($settings['cat_cover']) && !empty($c['cover'])) {
                $c['img_src'] = $this->getImgSrc('category', $c['id_category'], $settings['cat_cover'], $c['cover']);
            }
        }
        return $categories;
    }
    ...
    public function ajaxToggleParam()
    {
        ...
        $update_query = '
            ...
            SET '.bqSQL($param_name).' = \''.pSQL($param_value).'\'
    ...
    public function ajaxSave()
    {
        ...
        $ret = array('id' => $id, 'total' => $this->getTotal(Tools::strtolower($resource)));
        if ($resource == 'Category') {
            ...
            $ret['tree'] = utf8_encode($this->display(__FILE__, 'views/templates/admin/category-tree.tpl'));
        } else {
            $full = Tools::getValue('full');
            if ($resource == 'Post') {
                $ret['form'] = utf8_encode($this->callPostForm($id, $full));
            } elseif ($resource == 'Block') {
                $ret['form'] = utf8_encode($this->callBlockForm($id, '', $full));
            }
        }
        exit(Tools::jsonEncode($ret));
    }
    ...
    public function callCategoryForm($id_category, $id_parent = false)
    {
        ...
        $category['editable_fields']['images']['value'] = $this->getImages('category', $category['id_category'], true);

        $this->context->smarty->assign(array(
            'category' => $category,
            'image_types' => $this->getImgTypes('category'),
            'sorted_categories' => $this->getSortedCategories(),
            'root_id' => $this->root_id,
            'languages' => Language::getLanguages(false),
            'id_lang_current' => $this->id_lang,
    ...
    public function saveCategory($category_data)
    {
        ...
        $id_parent = $id_category != $this->root_id ? $category_data['id_parent'] : 0;
        $active = $id_category != $this->root_id ? $category_data['active'] : 1;
        $level_depth = 0;
        if ($id_parent) {
    ...
    public function callPostForm($id_post, $full = true)
    {
        ...
        if ($id_post && $post_data) {
            ...
            $post['editable_fields']['images']['value'] = $this->getImages('post', $id_post, true);
        }
        ...
        $this->context->smarty->assign(array(
            ...
            'languages' => Language::getLanguages(false),
            'id_lang_current' => $this->id_lang,
            'image_types' => $this->getImgTypes('post'),
    ...
    public function getBasicInfos($type, $included_ids = '', $q = '', $excluded_ids = '', $limit = '')
    {
        ...
        $main_query = $items = array();
        if ($included_ids = $this->formatIDs($included_ids)) {
            $main_query[] = pSQL($db_vars[1]).' IN ('.pSQL($included_ids).')';
        }
        if ($q) {
            $main_query[] = pSQL($db_vars[2]).' LIKE \'%'.pSQL($q).'%\'';
        }
        if ($main_query) {
            $id_lang = $this->context->language->id;
            ...
            if ($included_ids) {
                // DESC is used only if products are searched not only by IDs.
                // non-matched IDs have position 0, so we use DESC to put matched IDs first
                $order_by = 'ORDER BY FIELD('.pSQL($db_vars[1]).', '.pSQL($included_ids).')'.($q ? ' DESC' : '');
            } else {
                $order_by = '';
            }
    ...
    public function savePost($post_data)
    {
        ...
        if ($saved && $id_post) {
            ...
            $tags = !empty($post_data['multilang']['tags']) ? $post_data['multilang']['tags'] : array();
            $related_products = !empty($post_data['related_products']) ? $post_data['related_products'] : '';
            $saved &= $this->savePostTags($id_post, $tags);
            $saved &= $this->saveRelatedIDs('post', $id_post, $related_products);
        }
    ...
    public function verifyMethod($method_name, $obj = null)
    {
        $obj = $obj ? $obj : $this;
    ...
    public function getLastImageNum($resource_type, $resource_id)
    {
        $num = 0;
        if ($path = array_pop($this->getImages($resource_type, $resource_id))) {
            $num = current(explode('-', basename($path)));
        }
        return $num;
    }

    public function getImgSubDir($resource_type)
    {
        // not using singular names post/category for retro-compatibility
        return $resource_type == 'category' ? 'categories' : 'posts';
    }

    public function getAjaxResourceData()
    {
        $resource = array(
            'type' => Tools::getValue('resource_type'),
            'id' => (int)Tools::getValue('resource_id'),
        );
        if (($resource['type'] != 'post' && $resource['type'] != 'category') || !$resource['id']) {
            $this->throwError('error: resource not available');
        }
        return $resource;
    }

    public function addDirIfRequired($path)
    {
        if (!file_exists($path)) {
            mkdir($path, 0755);
    ...
    {
        $resource = $this->getAjaxResourceData();
        $path = $this->getImgSubDir($resource['type']).'/'.$resource['id'].'/';
        $this->addDirIfRequired($this->img_dir_local.$path);
        $saved_images = array();
        $num = $this->getLastImageNum($resource['type'], $resource['id']);
        $img_types = $this->getImgTypes($resource['type']);
        $special_imgs_verified = false;
        foreach ($_FILES as $file) {
            $saved_img = $this->saveSubmittedImage($file, $this->img_dir_local.$path, ++$num, $img_types);
            if (!$special_imgs_verified) {
                foreach (array_keys($this->special_img_types) as $type) {
                    if (!$this->getImg($resource['type'], $resource['id'], $type)) {
                        $this->setImg($resource['type'], $resource['id'], $type, $saved_img);
                    }
                }
                $special_imgs_verified = true;
            }
            $saved_images[$num] = $this->img_dir.$path.$saved_img;
        }
        ksort($saved_images);
        $smarty_array = array(
            'images' => $saved_images,
        ...
        foreach (array_keys($this->special_img_types) as $type) {
            $smarty_array[$type] = $this->getImg($resource['type'], $resource['id'], $type);
        }
        $this->context->smarty->assign($smarty_array);
        $saved_images_html = $this->display(__FILE__, 'views/templates/admin/available-images.tpl');
        $ret = array('saved_images_html' => utf8_encode($saved_images_html));
        exit(Tools::jsonEncode($ret));
    }

    public function saveSubmittedImage($file, $location, $prefix, $img_types, $keep_original = true)
    {
        ...
        foreach ($img_types as $t) {
            $path = $location.'/'.$t['dir'].'/';
            $this->addDirIfRequired($path);
            $path .= $new_img_name;
            if (!$this->imageResizeModified($location.$new_img_name, $path, $t['width'], $t['height'], $ext)) {
    ...
    public function getImg($resource_type, $resource_id, $img_type, $check_existence = true)
    {
        $img = $this->db->getValue('
            SELECT '.pSQL($img_type).' FROM '._DB_PREFIX_.'a_blog_'.pSQL($resource_type).'
            WHERE id_'.pSQL($resource_type).' = '.(int)$resource_id.'
        ');
        if ($img && $check_existence &&
            !is_file($this->img_dir_local.$this->getImgSubDir($resource_type).'/'.$resource_id.'/'.$img)) {
            $img = false;
        }
        return $img;
    }

    public function ajaxSetImg()
    {
        $resource = $this->getAjaxResourceData();
        $img_type = Tools::getValue('img_type');
        $img_name = Tools::getValue('img_name');
        $ret = array(
            'success' => $this->setImg($resource['type'], $resource['id'], $img_type, $img_name),
            $img_type => $img_name,
        );
        exit(Tools::jsonEncode($ret));
    }

    public function setImg($resource_type, $resource_id, $img_type, $img_name)
    {
        return $this->db->execute('
            UPDATE '._DB_PREFIX_.'a_blog_'.pSQL($resource_type).'
            SET '.pSQL($img_type).' = \''.pSQL($img_name).'\'
            WHERE id_'.pSQL($resource_type).' = '.(int)$resource_id.'
        ');
    }

    public function getImages($resource_type, $resource_id, $add_path = false)
    {
        $sorted = array();
        $path = $this->getImgSubDir($resource_type).'/'.$resource_id.'/';
        $images = glob($this->img_dir_local.$path.'*');
        foreach ($images as $img) {
            if (getimagesize($img)) {
                $img_name = basename($img);
                $key = current(explode('-', $img_name));
                $sorted[$key] = ($add_path ? $this->img_dir.$path : '').$img_name;
            }
        }
        ksort($sorted);
        return $sorted;
    ...
    public function ajaxDeleteImg()
    {
        $resource = $this->getAjaxResourceData();
        $img_name = Tools::getValue('img_name');
        $deleted = $this->deleteImg($resource['type'], $resource['id'], $img_name);
        $images = $this->getImages($resource['type'], $resource['id']);
        $current_img_name = current($images);
        foreach (array_keys($this->special_img_types) as $img_type) {
            if ($this->getImg($resource['type'], $resource['id'], $img_type, false) == $img_name
                && !empty($current_img_name)) {
                $this->setImg($resource['type'], $resource['id'], $img_type, $current_img_name);
            }
        }
        $ret = array(
            'success' => $deleted,
            'cover' => $this->getImg($resource['type'], $resource['id'], 'cover'),
            'main_img' => $this->getImg($resource['type'], $resource['id'], 'main_img'),
        );
        exit(Tools::jsonEncode($ret));
    }

    public function deleteImg($resource_type, $resource_id, $img_name)
    {
        if ($this->checkIfImgIsUsed($resource_type, $resource_id, $img_name)) {
            $this->throwError($this->l('It can not be deleted because it is used in text'));
        }
        $res = true;
        $dir = $this->img_dir_local.$this->getImgSubDir($resource_type).'/'.$resource_id.'/';
        if ($res = unlink($dir.$img_name)) {
            foreach (glob($dir.'*', GLOB_ONLYDIR) as $subdir) {
                $img_path = $subdir.'/'.$img_name;
                $res &= file_exists($img_path) ? unlink($img_path) : true;
            }
        }
        return $res;
    }

    public function checkIfImgIsUsed($resource_type, $resource_id, $img_name)
    {
        $column = $resource_type == 'post' ? 'content' : 'description';
        $identifier = 'id_'.$resource_type;
        return $this->db->getValue('
            SELECT '.pSQL($identifier).' FROM '._DB_PREFIX_.'a_blog_'.pSQL($resource_type).'_lang
            WHERE '.pSQL($identifier).' = '.(int)$resource_id.'
            AND '.pSQL($column).' LIKE  \'%/'.pSQL($img_name).'"%\'
         ');
    }
    ...
    public function getImgTypeData($id_img_type)
    {
        return $this->db->getRow('
            SELECT * FROM '._DB_PREFIX_.'a_blog_img_type WHERE id_img_type = '.(int)$id_img_type.'
        ');
    }

    public function ajaxRegenerateThumbnails()
    {
        $id_img_type = Tools::getValue('id_img_type');
        $resource_type = Tools::getValue('resource_type');
        $id_resource = (int)Tools::getValue('id_resource');
        $ret = array();
        // first run
        if (!$id_resource) {
            $ids = $this->db->executeS('
                SELECT id_'.pSQL($resource_type).' AS id
                FROM '._DB_PREFIX_.'a_blog_'.pSQL($resource_type).'
            ');
            foreach ($ids as &$id) {
                $id = $id['id'];
            }
            $ret['resource_ids'] = $ids;
        } elseif ($img_type_data = $this->getImgTypeData($id_img_type)) {
            $location = $this->img_dir_local.$this->getImgSubDir($resource_type).'/'.$id_resource.'/';
            $images = glob($location.'*');
            $total = 0;
            foreach ($images as $src_img) {
                if (getimagesize($src_img)) {
                    $img_name = basename($src_img);
                    $ext = pathinfo($img_name, PATHINFO_EXTENSION);
                    $path = $location.$img_type_data['dir'].'/';
                    $this->addDirIfRequired($path);
                    $dest_img = $path.$img_name;
                    $this->imageResizeModified(
                        $src_img,
                        $dest_img,
                        $img_type_data['width'],
                        $img_type_data['height'],
                        $ext
                    );
                    $total++;
                }
            }
            $ret['id_resource'] = $id_resource;
    ...
    public function getCategoryParents($id_category, $include_root = true, $include_current = true, $parents = array())
    {
        $cat = $this->db->getRow('
            SELECT c.id_category, c.id_parent, cl.title, cl.link_rewrite
            FROM '._DB_PREFIX_.'a_blog_category c
            LEFT JOIN '._DB_PREFIX_.'a_blog_category_lang cl
                ON c.id_category = cl.id_category AND cl.id_lang = '.$this->id_lang.'
                AND cl.id_shop = '.(int)$this->id_shop.'
            WHERE c.id_category ='.(int)$id_category.($include_root ? '' : ' AND c.level_depth > 0').'
        ');
        if ($cat) {
            if ($include_current) {
                $parents[] = $cat;
            }
            return $this->getCategoryParents($cat['id_parent'], $include_root, true, $parents);
        } else {
    ...
    public $parent_urls = array();

    public function getCategoryParentsURL($id_category)
    {
        $key = $this->id_lang.'_'.$id_category;
        if (!isset($this->parent_urls[$key])) {
            $this->parent_urls[$key] = '';
            foreach ($this->getCategoryParents($id_category, false) as $c) {
                $this->parent_urls[$key] .= $c['link_rewrite'].'/';
            }
        }
        return $this->parent_urls[$key];
    }

    public function getCategoryLink($id_category, $link_rewrite = false, $page = 1, $base_uri = true)
    {
        $url = $base_uri ? $this->getBaseUri() : '';
        if ($id_category != $this->root_id) {
            if ($this->friendly_url) {
                if (!empty($this->general_settings['url_parents'])) {
                    $url .= $this->getCategoryParentsURL($id_category);
                } elseif ($link_rewrite) {
                    $url .= $link_rewrite.'/';
                } else {
                    $url .= $this->getLinkRewriteById('category', $id_category).'/';
                }
            } else {
    ...
    public function getPostLink($id_post, $link_rewrite = false, $id_category_default = false, $base_uri = true)
    {
        $url = $base_uri ? $this->getBaseUri() : '';
        if ($this->friendly_url) {
            $link_rewrite = $link_rewrite ? $link_rewrite : $this->getLinkRewriteById('post', $id_post);
            if (!empty($this->general_settings['url_parents'])) {
                $id_category_default = $id_category_default ? $id_category_default : $this->getPostCategory($id_post);
                $url .= $this->getCategoryParentsURL($id_category_default);
            }
            $url .= $link_rewrite;
        } else {
    ...
    {
        $url = $this->getBaseUri();
        $tag_url = is_array($tag_url) ? implode('_', $tag_url) : $tag_url;
        if ($this->friendly_url) {
            $url .= 'tags/'.$tag_url.'/';
        } else {
    ...
    public function getNotAvailableLink()
    {
        return $this->getBaseUri().($this->friendly_url ? 'not-available/' : '?not-available');
    }
    ...
    public function getBaseUri($params = array())
    {
        if (empty($this->base_uri)) {
            $this->base_uri = $this->context->link->getModuleLink($this->name, 'blog', $params);
        }
    ...
    public function getPostCategory($id_post)
    {
        return $this->db->getValue('
            SELECT id_category_default FROM '._DB_PREFIX_.'a_blog_post
            WHERE id_post = '.(int)$id_post.'
        ');
    }
        ...
        if ($resource_type == 'post') {
            ...
            switch ($settings['type']) {
                ...
                case 'post_random':
                    ...
                    break;
                case 'post_selected':
                    $additional_filters['post_ids'] = !empty($settings['post_ids']) ? $settings['post_ids'] : '-1';
                    break;
                case 'post_relatedtoproduct':
                    $id_product = Tools::getValue('id_product');
                    $related_ids = $this->formatIDs($this->getRelatedIDs('product', $id_product));
                    $additional_filters['post_ids'] = $related_ids ? $related_ids : '-1';
                    $order_by = 'FIELD(p.id_post, '.$additional_filters['post_ids'].')';
                    $order_way = 'ASC';
                    break;
                case 'post_relatedtopost':
                        ...
                        foreach (array('category', 'tag') as $key) {
                            if (!empty($settings['related'][$key])) {
                                ...
                                if ($key_ids) {
                                    ...
                                    WHERE '.pSQL($identifier).' IN ('.pSQL($imploded_ids).')
            ...
            $pagination_settings = array('npp' => $settings['num'], 'p' => 1);
            ...
            if (!empty($this->context->controller->id_post)) {
                ...
                array_splice($related_ids, $settings['num']);
    ...
    {
        $this->redirectIfTrailingSlashIsMissing(); // main blog URL without trailing slash is not recognized by router
    ...
    public function addRoutesForOtherLanguages($params)
    {
        if (empty($this->slug_multilang)) {
            return;
        }
        $dispatcher = Dispatcher::getInstance();
        foreach ($this->slug_multilang as $id_lang => $slug) {
            if ($id_lang == $this->context->language->id && isset($this->context->controller->current_path)) {
                $path = $this->context->controller->current_path;
            } else {
                $this->id_lang = $id_lang; // use custom lang for parent urls
                $path = $params['resource_type'] == 'post' ?
                $this->getPostLink($params['id'], false, $params['post_category'], false) :
                $this->getCategoryLink($params['id'], false, $params['page'], false);
            }
            $dispatcher->addRoute(
    ...
    public function prepareHeaderData($params)
    {
        $this->addRoutesForOtherLanguages($params);
        $this->addJS($params['resource_type'].'.js');
        $this->addCSS($params['resource_type'].'.css');
        if ($params['resource_type'] == ' category') {
            if ($this->is_17) {
                $this->addCSS('category-17.css');
            }
        } elseif ($params['resource_type'] == 'post') {
            $this->addCSS('related-products.css');
        }
        Media::addJsDef(array(
            'ab_id_'.$params['resource_type'] => $params['id'],
    ...
    public function getDimensionsFromString($string)
    {
        ...
        $w = (int)$dimensions[0];
        $h = isset($dimensions[1]) ? (int)$dimensions[1] : $w;
    ...
    public function ajaxEditComment()
    {
        ...
        if ($avatar_new = $this->updateAvatar($comment_settings['avatar'], $avatar_prev)) {
            $this->db->execute('
                UPDATE '._DB_PREFIX_.'a_blog_user SET
                avatar = \''.pSQL($avatar_new).'\' WHERE avatar = \''.pSQL($avatar_prev).'\'
            ');
        }
    ...
    public function updateAvatar($size_string, $avatar_prev)
    {
        if (isset($_FILES['avatar_file'])) {
            $img_types = array(array('dir' => 'avatars') +  $this->getDimensionsFromString($size_string));
            $avatar = $this->saveSubmittedImage($_FILES['avatar_file'], $this->img_dir_local, false, $img_types, false);
            if ($avatar) {
    ...
    public function validateCaptcha($settings)
    {
        if (!empty($settings['captcha']) && !empty($settings['captcha_secret'])) {
            $data = array();
            if (function_exists('curl_init')) {
                ...
                $data = Tools::jsonDecode(curl_exec($ch), true);
                curl_close($ch);
            }
            if (empty($data['success'])) {
                $this->throwError($this->l('Captcha validation failed'));
            }
        }
    }
    ...
    public function ajaxSubmitComment()
    {
        ...
        $this->verifyIp($ip);
        $this->validateCaptcha($comment_settings);
        ...
        $ret = array(
            ...
            'instant_publish' => $comment_settings['instant_publish'],
        );
        exit(Tools::jsonEncode($ret));
    }
    ...
    public function saveUserData($id_guest, $user_name)
    {
        ...
        if ($avatar_new = $this->updateAvatar($comment_settings['avatar'], $user_data['avatar'])) {
            $user_data['avatar'] = $avatar_new;
    ...
    public function getValueAndValidate($val, $rule, $required = false, $max_chars = 256)
    {
        $value = Tools::getValue($val);
        if ($required && $value == '') {
            $this->errors[$val] = $this->l('Please, fill in this field');
        } elseif (is_callable(array('Validate', $rule)) && !Validate::$rule($value)) {
    ...
    public function getListItemTplPath($type)
    {
        return $this->getTemplatePath($type.'-list-item.tpl');
    }
    ...
    {
        // Language::loadUpdateSQL() doesn't work because id_post should be id_a_blog_post
        $this->fillMissingLanguages();
    }

    public function getAvailableLanguageIds($id_shop = false)
    {
        // ids_only as argument is not available in some retro versions
    ...
    public function fillMissingLanguages()
    {
        Language::loadLanguages();
        foreach (array('post', 'category', 'block') as $resource_type) {
            $identifier = 'id_'.$resource_type;
            $table_name = 'a_blog_'.pSQL($resource_type).'_lang';
            $data = $this->db->executeS('SELECT * FROM '._DB_PREFIX_.pSQL($table_name));
            $sorted_data = $new_rows = array();
            foreach ($data as $d) {
                $sorted_data[$d['id_shop']][$d[$identifier]][$d['id_lang']] = $d['id_lang'];
            }
            foreach ($sorted_data as $id_shop => $resources) {
                $id_lang_default = Configuration::get('PS_LANG_DEFAULT', null, null, $id_shop);
                $lang_ids = $this->getAvailableLanguageIds($id_shop);
                foreach ($resources as $id_resource => $saved_translations) {
                    $missing_translations = array_diff($lang_ids, $saved_translations);
                    foreach ($missing_translations as $id_lang) {
                        $default_lang_data = $this->db->getRow('
                            SELECT * FROM '._DB_PREFIX_.pSQL($table_name).'
                            WHERE '.pSQL($identifier).' = '.(int)$id_resource.'
                            AND id_shop = '.(int)$id_shop.' AND id_lang = '.(int)$id_lang_default.'
                        ');
                        $default_lang_data['id_lang'] = $id_lang;
                        foreach ($default_lang_data as $col_name => &$val) {
                            $allow_html = in_array($col_name, array('content', 'description'));
                            $val = pSQL((_PS_MAGIC_QUOTES_GPC_ ? addslashes($val) : $val), $allow_html);
                        }
                        $new_rows[] = '(\''.implode('\', \'', $default_lang_data).'\')';
                    }
                }
            }
            if ($new_rows) {
                $this->db->execute('
                    REPLACE INTO '._DB_PREFIX_.pSQL($table_name).' VALUES '.implode(', ', $new_rows).'
                ');
            }
        }
    }
    ...
    public function convertImgSettingsToNewFormat($retro_settings)
    {
        $retro_names = array(
            'xs' => $this->l('Extra small'),
            's'  => $this->l('Small'),
            'm'  => $this->l('Medium'),
            'l'  => $this->l('Large'),
            'xl' => $this->l('Extra large'),
        );
        $rows = array();
        foreach ($retro_settings as $dir => $size) {
            $size = $this->getDimensionsFromString($size);
            $desc_name = isset($retro_names[$dir]) ? $retro_names[$dir] : $dir;
            $rows[] = '(0, \''.pSQL($desc_name).'\', \''.pSQL($dir).'\',
            '.(int)$size['width'].', '.(int)$size['height'].', 1, 1)';
        }
        $this->db->execute('
            REPLACE INTO '._DB_PREFIX_.'a_blog_img_type VALUES '.implode(', ', $rows).'
        ');
    }

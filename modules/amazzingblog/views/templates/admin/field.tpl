...
{else if $type == 'switcher'}
	<span class="switch prestashop-switch">
		<input type="radio" id="{$id|escape:'html':'UTF-8'}" name="{$name|escape:'html':'UTF-8'}" value="1"{if !empty($field.value)} checked{/if}{if !empty($field.readonly)} disabled{/if} >
		<label for="{$id|escape:'html':'UTF-8'}">{l s='Yes' mod='amazzingblog'}</label>
		<input type="radio" id="{$id|escape:'html':'UTF-8'}_0" name="{$name|escape:'html':'UTF-8'}" value="0"{if empty($field.value)} checked{/if}{if !empty($field.readonly)} disabled{/if} >

...
class BlogFields extends AmazzingBlog
{
    public function getPostFields($only_multilang = false)
    {
        ...
        if ($only_multilang) {
            unset($fields['tags']); // tags are processed separately
    ...
    public function getPostSettingsFields()
    {
        $fields = array(
            ...
            'shop_name_in_mt' => array(
                'display_name' => $this->l('Include shop name in meta title'),
                'type' => 'switcher',
                'value' => 0,
            ),
            'main_img_type' => array(
    ...
    public function getPostlistSettingsFields()
    {
        $fields = array(
            ...
            'npp' => array(
                ...
                'value' => $this->getDefaultListSettings('npp'),
            ),
            'npp_options' => array(
                'display_name' => '',
                'value' => $this->getDefaultListSettings('npp_options'),
                'class' => 'hidden',
            ),
            'order_by' => array(
    ...
    public function getCategoryFields($id_parent = false, $only_multilang = false)
    {
        ...
        foreach ($fields as $k => $f) {
            if (!empty($f['multilang'])) {
                $fields[$k]['input_name'] = 'multilang['.$k.']';
            } elseif ($only_multilang) {
                unset($fields[$k]);
            }
        }
        return $fields;
    ...
    public function getCategorySettingsFields()
    {
        $fields = array(
            ...
            'main_img' => array(
                'display_name' => $this->l('Category main image size'),
                'options' => $this->getImgOptions('category'),
            ),
            ...
            'subcat_cover' => array(
                'display_name' => $this->l('Cover image size for subcategories'),
                'options' => $this->getImgOptions('category'),
            ),
            'include_all' => array(
        ...
        foreach ($fields as &$f) {
            $f['value'] = isset($f['value']) ? $f['value'] : 0;
        }
        return $fields;
    }

    public function getImgOptions($scope = 'post')
    {
        $options = array('0' => $this->l('None'));
        foreach ($this->getImgTypes($scope) as $t) {
            $options[$t['dir']] = $t['name'].' ('.$t['width'].'*'.$t['height'].')';
        }
        return $options;
    }
    ...
    public function getBlockFields()
    {
        ...
        $fields = array(
            ...
            'cat_cover' => array(
                'display_name'  => $this->l('Thumbnail'),
                'input_name' => 'settings[cat_cover]',
                'value' => 0,
                'options' => $this->getImgOptions('category'),
                'class' => 'resource-type-option category hidden',
            ),
            'class' => array(
    ...
    {
        $captcha_note = $this->l('Site and secret keys can be obtained %s');
        $captcha_link = '<a href="https://www.google.com/recaptcha/admin" target="_blank">'.$this->l('here').'</a>';
        $captcha_note = sprintf($captcha_note, $captcha_link).'<br>'.$this->l('Select "Invisible reCAPTCHA"');
        $fields = array(
            'avatar' => array(
                'display_name' => $this->l('Avatar dimentions'),
                'tooltip' => $this->l('For example: 55*55'),
                'value' => '55*55',
                'validate' => 'isAvatarSize',
                'required' => 1
            ),
            'max_chars' => array(
            ...
            'allow_html' => array(
                'display_name' => $this->l('Allow basic HTML in comments'),
                'tooltip' => $this->l('Bold, italics, underline, emoticons'),
                'value' => 1,
                'type' => 'switcher',
            ),
            'captcha' => array(
                'display_name' => $this->l('Activate reCAPTCHA'),
                'value' => 0,
                'type' => 'switcher',
            ),
            'captcha_key' => array(
                'display_name' => $this->l('reCAPTCHA Site key'),
                'value' => '',
                'class' => 'captcha-option hidden',
                'required_if' => 'captcha',
                'validate' => 'isGenericName',
            ),
            'captcha_secret' => array(
                'display_name' => $this->l('reCAPTCHA Secret key'),
                'tooltip' => $this->l('Do not share it with anyone'),
                'value' => '',
                'help_box' => array($captcha_note),
                'class' => 'captcha-option hidden',
                'required_if' => 'captcha',
                'validate' => 'isGenericName',
            ),
        ...
        if (!function_exists('curl_init')) {
            $fields['captcha']['readonly'] = 1;
            $fields['captcha']['warning'] = $this->l('This option requires cURL extension');
        }
        return $fields;
    }
    ...
    public function getGeneralSettingsFields()
    {
        $fields = array(
            ...
            'url_parents' => array(
                'display_name' => $this->l('Include category parents in URLs'),
                'type' => 'switcher',
                'value' => 1,
            ),

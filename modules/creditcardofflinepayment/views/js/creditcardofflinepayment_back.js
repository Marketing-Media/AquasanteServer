...
jQuery('document').ready(function() {
	...
	$.each(fields, function(index, value) {
		...
		if (field.val() == 1) {
			field.parent().parent().parent().next().slideDown();
		} else {
		 	field.parent().parent().parent().next().slideUp();
		}
		...
		field.change(function() {
			if ($(this).val() == 1) {
				...
				if ($(this).attr('name') == 'CCOFFLINE_REQUIREEXP') {
					$('#CCOFFLINE_YEARS').parent().parent().slideDown();
				}
			} else {
				field.parent().parent().parent().next().slideUp();
				if ($(this).attr('name') == 'CCOFFLINE_REQUIREEXP') {
					$('#CCOFFLINE_YEARS').parent().parent().slideUp();
				}
			}
		});
	});
...
function showHidePaymentStyle() {
	if (jQuery('select[name="CCOFFLINE_PAYMENT_STYLE"]').val() == 1) {
		...
		$('select[name="CCOFFLINE_YEARS"]').parent().parent().slideDown();
		...
		$('select[name="CCOFFLINE_YEARS"]').parent().parent().slideUp();
	}
}
...
function showHideWorkingMode() {
	if (jQuery('input[name="CCOFFLINE_WORKING_MODE"]:checked').val() == 1) {
		...
		jQuery('input[name="CCOFFLINE_ADMIN_MAIL"]').parent().parent().slideDown();
	} else if (jQuery('input[name="CCOFFLINE_WORKING_MODE"]:checked').val() == 2) {
		jQuery('#working_mode_by_mail').slideUp();
		jQuery('input[name="CCOFFLINE_ADMIN_MAIL"]').parent().parent().slideUp();
